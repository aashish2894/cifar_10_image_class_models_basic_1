...
X_train_red = np.dot(X_train,U[:,:150])
X_val_red = np.dot(X_val,U[:,:150])
X_test_red = np.dot(X_test,U[:,:150])
...
for lr in learning_rates:
	for rs in regularization_strengths:
		...
		net = TwoLayerNet(input_size, hidden_size, num_classes)
		# Train the network
		stats = net.train(X_train_red, y_train, X_val_red, y_val,num_iters=iters, batch_size=500,learning_rate=lr, learning_rate_decay=0.95,reg=rs, verbose=True)
		y_train_pred = net.predict(X_train_red)
		acc_train = np.mean(y_train == y_train_pred)
		y_val_pred = net.predict(X_val_red)
...
test_acc = (best_net.predict(X_test_red) == y_test).mean()
